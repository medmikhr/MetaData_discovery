...
def read_sql_query(sql_file = 'physical_tables'):
    with open(FP['sql'] + sql_file + '_v.sql', 'r') as file:
        query = file.read()
    file.close()
    return query
...
def write_df_old(df, system, entity = 'physical_tables'):
...
def write_df(df, system, db, entity = 'physical_tables'):
    if entity == 'physical_tables':
        df['system'] = system
        df['dbms'] = 'postgres'
    else:
        df['physical_table'] = db + '_' +  df['physical_table']
    df['id'] = system + '_' + db + '_' + df['id']
    df = df.set_index('id')
    output_path = FP['csv'] + entity + '.csv'
    header_switch = True if os.path.getsize(os.path.join(os.getcwd(), output_path)) == 0 else False
    df.to_csv(output_path, mode='a', header=header_switch)
...
def create_ph_level (con_cursor, entity, system, db):
    sql_query = read_sql_query(entity)
    sql_output = fetch_query_data(con_cursor, sql_query)
    df_output = write_df(sql_output, system, db, entity)
    return df_output
